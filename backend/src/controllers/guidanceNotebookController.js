...
// ==========================================
// OBTER CADERNO PELO PRÉ-PROJECTO
// ==========================================

exports.getNotebookByPreProject = async (req, res) => {
  try {
    const { preProjectId } = req.params;

    if (!preProjectId) {
      return res.status(400).json({
        success: false,
        message: 'preProjectId é obrigatório.'
      });
    }
    ...
    if (notebooks.length === 0) {
      return res.status(404).json({
        success: false,
        message:
          'Não existe um caderno de orientação associado a este pré-projecto.'
      });
    }
    ...
    console.error(
      'Erro ao buscar caderno pelo pré-projecto:',
      error
    );

    return res.status(500).json({
      success: false,
      message: 'Erro ao carregar o caderno de orientação.',
      error: error.message
    });
  }
};